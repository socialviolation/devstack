...
func registerHooksTool(mcpServer *server.MCPServer, ws *workspace.Workspace) {
	tool := mcp.NewTool("hooks",
		mcp.WithDescription("Inspect and test lifecycle hooks. A hook is a shell command that devstack runs on its own when a stack or a service changes state.\n"+
			...
			"Hooks are declared in devstack.workspace.yaml, which the team shares. A 'services:' list there scopes a hook to named services. Hooks are also declared in a service's devstack.service.yaml, and those cover that service only. A feature stack inherits the workspace's hooks.\n\n"+
