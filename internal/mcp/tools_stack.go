...
func registerStackAddTool(mcpServer *server.MCPServer, ws *workspace.Workspace) {
	...
	mcpServer.AddTool(tool, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		_, genWarnings, err := hostdaemon.RegenerateScope(hostdaemon.ScopeStack(ws.Name, res.Namespace))
		if err != nil {
...
func registerStackUpTool(mcpServer *server.MCPServer, ws *workspace.Workspace) {
	...
	mcpServer.AddTool(tool, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		_, genWarnings, err := hostdaemon.RegenerateScope(hostdaemon.ScopeStack(ws.Name, rec.Name))
		if err != nil {
