...
// A resource absent from the view has not settled: a restart regenerates the
// Tiltfile, so a resource can vanish mid-reload, and treating that as settled
// reports a restart as complete while the old process may still be up. The wait
// is bounded, so it ends at the timeout and says what it could not see.
func TestWaitDoesNotClaimAVanishedResourceSettled(t *testing.T) {
	fetch := func() (*tilt.TiltView, error) { return safetyView(safetyResource("navexa:other", "ok", "t1")), nil }
	states, settled, err := coreWaitForSettled(fetch, []string{"navexa:api"}, nil, 3*time.Second, time.Second, func(time.Duration) {})
	if err != nil {
		t.Fatal(err)
	}
	if settled {
		t.Fatalf("a resource that is not in the daemon must not read as settled: %#v", states)
	}
	if states[0].found {
		t.Errorf("resource should be marked absent: %#v", states[0])
	}
	if report := coreWaitReport(states, settled, 3*time.Second); !strings.Contains(report, "not in the daemon") {
		t.Errorf("report should say the resource was not visible: %q", report)
	}
}
...
// A wrong "auto" tells an agent its edit is already live when the old code is
// still running, so ambiguous tokens must match as whole words only.
func TestSafetyWatchCommandDoesNotMatchSubstrings(t *testing.T) {
	auto := []string{
		"dotnet watch run", "npm run dev", "vite", "vite --host", "air",
		"uvicorn app:app --reload", "nodemon server.js", "ng serve",
	}
	for _, cmd := range auto {
		if !coreWatchCommand(cmd) {
			t.Errorf("%q should classify as self-watching", cmd)
		}
	}

	manual := []string{
		"vitess-server --config x", "go run ./cmd/airflow", "dotnet run",
		"sleep 3600", "./bin/repair --hotfix", "python manage.py runserver --noreload",
	}
	for _, cmd := range manual {
		if coreWatchCommand(cmd) {
			t.Errorf("%q must not classify as self-watching", cmd)
		}
	}
}

// configure writes an argument list the Tiltfile has to opt into reading. The
// generated Tiltfile never does, so a write there restarts services and changes
// nothing — refuse it rather than appear to work.
func TestSafetyConfigureRefusesWhenTiltfileReadsNoArgs(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	generated := "# GENERATED BY devstack — DO NOT EDIT.\nlocal_resource('api', serve_cmd='go run .')\n"
	if err := os.WriteFile(filepath.Join(dir, "Tiltfile"), []byte(generated), 0644); err != nil {
		t.Fatal(err)
	}
	if coreTiltfileReadsArgs() {
		t.Error("a Tiltfile with no config.define_/config.parse must report that it reads no arguments")
	}

	withArgs := generated + "cfg = config.parse()\n"
	if err := os.WriteFile(filepath.Join(dir, "Tiltfile"), []byte(withArgs), 0644); err != nil {
		t.Fatal(err)
	}
	if !coreTiltfileReadsArgs() {
		t.Error("a Tiltfile calling config.parse does read arguments")
	}
}
