...
func registerMigrateTool(mcpServer *server.MCPServer, patches []migrate.Patch) {
	tool := mcp.NewTool("migrate",
		mcp.WithDescription("Run every migration that this devstack needs, or preview them. A migration is one versioned unit of work with its own detector. devstack runs each pending migration over every workspace on this machine, and not over this workspace alone.\n"+
			"action=\"list\" reads only. It prints each migration, applied or pending, with the reason. It changes no file.\n"+
			"action=\"run\" applies each pending migration. It changes files.\n"+
			"CAUTION: devstack does not own the service repositories. A run writes .mcp.json and .claude/settings.json in each repository of each workspace. It removes the devstack block from AGENTS.md, CLAUDE.md, GEMINI.md, .cursorrules and .github/copilot-instructions.md. It deletes a file that holds that block and nothing else. It also cuts one git worktree for each repository, to build the replica that base runs from. devstack removes only what devstack wrote. Your own text stays, byte for byte.\n"+
			...
			"- Restart the services that run from a new replica.\n"+
		...
		mcp.WithString("action", mcp.Required(),
			mcp.Description("\"list\" prints each migration, applied or pending, and it changes nothing. Use it first: it reports what a run changes. \"run\" applies each pending migration. It writes and deletes files in repositories that devstack does not own, and it cuts git worktrees.")),
