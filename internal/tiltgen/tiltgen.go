...
type Options struct {
	...
	// SkipFreePorts names ports a service must not reclaim, keyed by service name.
	// GenerateHost fills it for a reclaim that would kill another supervised
	// resource, so the collision costs one service its reclaim instead of costing
	// every workspace on the machine its Tiltfile.
	SkipFreePorts map[string][]int
}

// skipping returns a copy of the options with one more suppressed reclaim.
func (o Options) skipping(service string, port int) Options {
	next := map[string][]int{}
	for k, v := range o.SkipFreePorts {
		next[k] = append([]int(nil), v...)
	}
	next[service] = append(next[service], port)
	o.SkipFreePorts = next
	return o
}

func (o Options) skipSet(service string) map[int]bool {
	if len(o.SkipFreePorts[service]) == 0 {
		return nil
	}
	set := make(map[int]bool, len(o.SkipFreePorts[service]))
	for _, p := range o.SkipFreePorts[service] {
		set[p] = true
	}
	return set
}
...
//
// It also returns a warning for each freePorts reclaim it dropped. A reclaim
// that would kill another supervised resource is suppressed for the service that
// declares it, and every other workspace still renders.
func GenerateHost(workspaces []WorkspaceGen) (string, []string, error) {
	conflicts, err := FreePortConflicts(workspaces)
	if err != nil {
		return "", nil, err
	}
	workspaces, warnings := dropConflictingReclaims(workspaces, conflicts)
...
func renderService(svc config.ResolvedService, ws *config.WorkspaceManifest, groups []string, opts Options, book config.PortBook, prefix, namespace string) (string, error) {
	...
	prep, err := prepCommand(m, svc.Name, book, opts.skipSet(svc.Name))
	if err != nil {
		return "", err
	}
	if prep != "" {
...
func prepCommand(m *config.ServiceManifest, service string, book config.PortBook, skip map[int]bool) (string, error) {
	var parts []string
