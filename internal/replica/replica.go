...
func Sync(ws *workspace.Workspace) (*SyncResult, error) {
	...
	for _, r := range repos {
		path := r.Path(root)
		if _, err := os.Stat(path); err != nil {
			res.Warnings = append(res.Warnings, fmt.Sprintf("the services %s have no worktree at %s. To build it, run devstack base sync", strings.Join(r.Services, ", "), path))
			continue
		}
...
var ErrNotBuilt = errors.New("devstack has not built the replica. To build it, run devstack base sync. To build it and start its services, run devstack workspace up")
